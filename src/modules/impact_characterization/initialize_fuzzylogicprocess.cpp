...
void init_flp_params(Fuzzy* &fuzzy){ // to call: init_flp_params(fuzzy);
	...
	FuzzyRuleAntecedent* ifAccVeryLow = new FuzzyRuleAntecedent();
	ifAccVeryLow->joinSingle(accVeryLow);	
	
	FuzzyRule* fuzzyRuleSet1Rule1 = new FuzzyRule(1,ifAccVeryLow,thenLevel);
	fuzzy->addFuzzyRule(fuzzyRuleSet1Rule1);
	...
	// Rule Set 1, Rule 6 (Acc Low & Level = Level output)
	FuzzyRuleAntecedent* ifAccLowAndInclinedLevel = new FuzzyRuleAntecedent();
	ifAccLowAndInclinedLevel->joinWithAND(accLow, inclinedLevel);		
	
	FuzzyRule* fuzzyRuleSet1Rule6 = new FuzzyRule(1,ifAccLowAndInclinedLevel,thenLevel);
	fuzzy->addFuzzyRule(fuzzyRuleSet1Rule6);

	// Rule Set 1, Rule 7 (Acc Med/High & Level = Towards Small output)
	FuzzyRuleAntecedent* ifAccHighAndInclinedLevel = new FuzzyRuleAntecedent();
	ifAccHighAndInclinedLevel->joinWithAND(accHigh, inclinedLevel);	
	
	FuzzyRuleAntecedent* ifAccMedAndInclinedLevel = new FuzzyRuleAntecedent();
	ifAccMedAndInclinedLevel->joinWithAND(accMedium, inclinedLevel);		

	FuzzyRuleAntecedent* ifAccMedOrHighAndInclinedLevel = new FuzzyRuleAntecedent();
	ifAccMedOrHighAndInclinedLevel->joinWithOR(ifAccMedAndInclinedLevel, ifAccHighAndInclinedLevel);	
	
	FuzzyRule* fuzzyRuleSet1Rule7 = new FuzzyRule(1,ifAccMedOrHighAndInclinedLevel,thenTowardSmall);
	fuzzy->addFuzzyRule(fuzzyRuleSet1Rule7);
